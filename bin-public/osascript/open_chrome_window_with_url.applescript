on run argv
    ...
    set urlToOpen to item 4 of argv

    -- Parse position string into a list
    set positionToOpen to my parsePositionString(positionToOpenStr)

    log "Preparing to open Chrome window with URL: " & urlToOpen & ", position: " & positionToOpenStr & ", profile: " & profile
    ...
    return "Successfully opened Chrome window with URL " & urlToOpen
end run
