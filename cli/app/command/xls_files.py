...
def get_commit_timestamp(args: Tuple[str, int, int]) -> Tuple[int, str]:
    ...
    last_commit_epoch = int(run_git_command(["git", "log", "-1", "--format=%ct", file]))
...
def get_commit_count(args: Tuple[str, int, int]) -> Tuple[int, str]:
    ...
    file, current, total = args
    show_progress("Processing files", current, total)
    commit_count = int(run_git_command(["git", "rev-list", "--count", "HEAD", file]))
...
def xls_files(sort: str, limit: int, ext: str | None):
    ...
    Examples:
        ...
        xls-files --sort edits --limit 20 --ext py
    ...
    if ext:
        ...
        files = run_git_command(["git", "ls-files", f"*.{ext}"]).split("\n")
    else:
        click.secho("Listing all files", fg="blue")
        files = run_git_command(["git", "ls-files"]).split("\n")
    ...
    with ThreadPoolExecutor() as executor:
        ...
        file_data = process_files_in_parallel(
            executor, get_commit_timestamp if sort == "time" else get_commit_count, files, total_files
        )
        ...
        # Format output
        start_time = time.time()
        formatter = get_formatted_timestamp if sort == "time" else get_formatted_count
