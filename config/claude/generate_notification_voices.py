...
async def generate_voice_file(
    voice: str, phrase: str, phrase_index: int, base_dir: Path
):
...
async def generate_voices(target_voice=None):
    ...
    # Create base directory for voice files in dotfiles structure
    base_dir = Path(__file__).parent / "voice_notifications"
    ...
    if target_voice:
        ...
        print(
            f"Generating {len(NOTIFICATION_PHRASES)} phrases for {target_voice} voice = {len(NOTIFICATION_PHRASES)} files"
        )
    else:
        voices_to_generate = AVAILABLE_VOICES
        print(
            f"Generating {len(NOTIFICATION_PHRASES)} phrases × {len(AVAILABLE_VOICES)} voices = {len(NOTIFICATION_PHRASES) * len(AVAILABLE_VOICES)} files"
        )
...
def create_voice_index(base_dir: Path):
    ...
    with open(index_file, "w") as f:
        ...
        f.write("## Usage\n\n")
        f.write(
            "These audio files are used by the notification hook to provide natural voice alerts.\n"
        )
        f.write(
            "The hook randomly selects a voice and phrase combination for variety.\n"
        )
